...
int main() {
    ...
    if (choice == '1') {
        ...
        // Create array to hold custom number of variables
        double custom_table[custom];
        cout<<endl;
        ...
        // Simple for loop for calculating the sum of the numbers
        for (int i = 1; i < custom; i++) {
            cout<<" + "<<custom_table[i];
            custom_average += custom_table[i];
        }

        // Finish printing the average in a UserFriendly format
        cout<<" ÷ "<<custom<<" = ";
        custom_average = custom_average / custom;
